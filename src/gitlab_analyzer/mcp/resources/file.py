...
async def _get_file_analysis(
    project_id: str, job_id: str, file_path: str, response_mode: str = "balanced"
) -> str:
    """Internal function to get file analysis with configurable response mode."""
    try:
        cache_manager = get_cache_manager()
        analyzer = get_gitlab_analyzer()

        # Create cache key for file analysis (include response mode)
        cache_key = f"file_errors_{project_id}_{job_id}_{file_path.replace('/', '_')}_{response_mode}"
        ...
        for entry in log_entries:
            ...
            if (
                ...
                or (hasattr(entry, "file_path") and file_path in str(entry.file_path))
        ...
        result = {
            ...
            "resource_uri": f"gl://file/{project_id}/{job_id}/{file_path}?mode={response_mode}",
...
def register_file_resources(mcp) -> None:
    ...
    async def get_file_resource(project_id: str, job_id: str, file_path: str) -> str:
        ...
        For different detail levels, use the get_file_errors tool with response_mode parameter.
        """
        return await _get_file_analysis(project_id, job_id, file_path, "balanced")

    @mcp.resource("gl://file/{project_id}/{job_id}/{file_path}?mode={mode}")
    async def get_file_resource_with_mode(
        project_id: str, job_id: str, file_path: str, mode: str
    ) -> str:
        """
        Get file error analysis as a resource with specific response mode.
        ...
        Returns:
            File error analysis optimized for the specified mode
        """
        return await _get_file_analysis(project_id, job_id, file_path, mode)
